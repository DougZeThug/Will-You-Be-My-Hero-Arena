...
The cornhole throw is how a player sends one bag at the board in a Play Cornhole match. They pick a spot, aim, choose a *shot*, hold to charge, and let go while the marker is inside the green *release window*. A match has one to four players. Each player throws four bags, taking turns in slot order. Every bag scores for the player who threw it:
- 3 in the hole
- 1 on the board
- 0 off it
...
- the *caption* reads "{name}: aim, hold charge, then release · Shot: {shot}", naming the shot that will be thrown
...
- the prompt "Doug: aim, hold charge, then release · Shot: Hole runner"

Moving left or right shifts the thrower along the throwing line. The aim keys or stick move the reticle.

The player presses and holds the charge button: Space on keyboard 1. The **RELEASE TIMING** meter appears at the bottom left of the stage, with a marker sliding right and a teal band that the caption calls the green window, and the character plays a short pre-throw ritual. The caption reads "Release in the green window · Shot: Hole runner". The player lets go while the marker is in the green. The caption reads **Perfect release**, the character throws, and a moment later the bag leaves the hand and arcs onto the board.

When the bag lands, the result is announced ("Doug: on the board — one point") and the thrower reacts. The nameplates update. After a short pause the next player's turn begins with the reticle re-centred. When every player has thrown four bags, the caption reads **Final score**, and the result shows "{name} wins", or **Draw** on a tie. The result offers **Play again** and **Choose another event**.
...
stateDiagram-v2
    ...
    charging --> aiming : pause, window loses focus, graphics lost, controller disconnects (charge discarded)
...
- **The shot is reset** to this character's favourite of the four shots (see [shots](#shots)). The reticle returns to the centre of the board. The thrower's spot along the line is *not* reset; it is wherever this character last stood.
- **The caption** changes to "{name}: aim, hold charge, then release · Shot: {shot}", for example "Dan: aim, hold charge, then release · Shot: Roll".
...
- **Choose a shot** with J, K, L or E: **Hole runner**, **Slide**, **Roll** or **Airmail**. The caption's "Shot:" part changes at once to the new shot.
...
- **The caption** reads "Release in the green window · Shot: {shot}". Their nameplate's status line reads **CHARGING**.
...
- **Changing the shot** changes the window width, the arc, and which throwing animation will play. The caption names the new shot at once.
...
- **Roll.** A **Roll** that nudges one or more bags curls around them once: it stops 16 pixels nearer the front edge, however many bags it nudged. That can carry it off the board.
...
- **The result** reads "{name} wins" for one winner, or **Draw** for a tie.
...
Each turn starts on the character's favourite of these four shots, so the starting shot can always be chosen again with its key:
- **Doug** starts on **Hole runner**.
- **Dan** starts on **Roll**.
- **Installed characters** start on the favourite of the four in their own throwing style. A character whose style favours none of them starts on **Hole runner**.

## Modifiers

| Modifier | Set at the start | Changed while committed |
| --- | --- | --- |
| Input device | **Keyboard 1** charges with Space, and **keyboard 2** with Enter. A **controller** charges with the right trigger (RT or R2). The trigger is analog, so a half pull counts once it passes 0.2 and releases below 0.1. **Touch** charges with a tap on **Hold / release**, which then reads **Release**. A second tap throws. The button can only be tapped during the player's own aiming or charging turn. An **AI player** charges 0.4 s after its turn starts, and lets go near the ideal, offset by up to 0.03 in a fixed pattern that follows that thrower's own bag count, so every AI thrower gets a similar spread of early and late releases ([AI players](../cross-cutting/ai-players.md)). | The device cannot be changed during a match. Adding on-screen taps to a keyboard or controller merges the two: see [input device changes](#cancel-and-interrupt). |
| Event and action combinations | **J/K/L/E** choose the shot, and the caption names it. **Right modifier** (left Ctrl, right Ctrl for keyboard 2, RB or R1) turns on precision mode while aiming or charging. It costs 15 stamina, lasts 3 s, and then has a 10 s cooldown; with less than 15 stamina it does nothing. The caption shows **Precision mode**. The **left modifier** (Shift, LB or L1) does nothing in cornhole. **Celebrate** (C) plays a celebration for any player at any time their character is not already mid-animation. J during the result pause does the same for the thrower. | The shot can still be changed; the latest choice before release wins. Precision mode can be turned on mid-charge, and the green band widens at once. It only narrows scatter if it is still active when the bag leaves the hand, 3 s after activation. Moving mid-charge moves the green band. |
...
| Character card | The character's Play accuracy sets the scatter: Dan 0.76, Doug 0.68. It is not the card's printed **Accuracy** bar (Dan 64, Doug 68). Its shot skills set the window width (see [shots](#shots)). Its favourite of the four shots is the one each turn starts on. **Dan** is a clutch performer, so his window is 0.025 wider on his fourth bag. Both built-in cards have precision mode. An installed character without a connected rig cannot start a match: see [interactions](#interactions-with-other-systems). | Not applicable: cards are chosen in setup. |
| Presentation settings | **Reduced motion** skips the camera punch on a hole, draws no impact effects at all, and turns off squash and stretch. **Lower graphics quality** has no effect in Play. The **clean spectator view** is Watch-only. Play's sound switch reads **Sound on** and starts off. Turned on, it plays a tone as the bag leaves the hand, a hole tone and a board-impact tone. | Toggling **Reduced motion** in Arena settings applies to the running match at once, mid-charge included: effects, the camera punch and squash follow the new setting, and nothing restarts. The sound switch takes effect on the next cue. |
...
| Saved state | The throw reads nothing from this browser's save. The only saved thing that matters is the bindings saved for the player's keyboard layout or controller at the last **Start**, which decide which keys are J/K/L/E and charge. | No effect. |

## Cancel and interrupt

| Event | Before committing | While committed |
| --- | --- | --- |
| Escape or click outside | **Escape** is keyboard 1's pause key. If a player uses keyboard 1, it pauses while stage focus is inside the stage, and does nothing otherwise. With no keyboard 1 player it does nothing. A **click elsewhere on the page** moves stage focus away, so later key presses do nothing. The turn waits. Clicking the stage gives it focus back. | **Escape** pauses and discards the charge. A **click elsewhere** does not stop the charge. The charge key's release still counts, so letting go throws. Otherwise the automatic release throws at 2.2 s. |
| Pause or resume | The game freezes and the **PAUSED** overlay shows **Resume when you’re ready.** The reticle, shot, spot and precision timer are kept. Resuming continues aiming. | The charge is **discarded**. The meter disappears, the ritual stops, and the player is back to aiming with no bag thrown. The throw counts for nothing and the same player still has the turn. The caption goes back to the aiming prompt, "{name}: aim, hold charge, then release · Shot: {shot}". After resuming they must press charge afresh; a key or trigger still held through the pause does nothing until released. Controllers must first [return to neutral](../foundations/input-model.md#neutral). |
...
| Settings or saved data change underneath | Toggling **Reduced motion** applies to the running match at once. Bags, scores, the turn and the sound are kept. Changing the scoring policy, **Reset demo**, or another tab writing the save have no effect on the match. | The charge carries on; only the effects change. |
| Graphics or storage failure | A lost WebGL context pauses the match with **PAUSED** and **Graphics were interrupted. Resume when the stage is back.** When the context returns, the notice becomes **Graphics are back. Resume when ready.**, and **Resume game** continues aiming. Saving bindings at **Start** fails silently if storage is refused. It happens before the throw and does not affect it. | The pause discards the charge, as for any pause. |
...
**Saved data and recovery.** No interaction. A throw writes nothing. The only Play data saved is the bindings, written for each keyboard layout or controller in use when **Start Cornhole** is pressed ([controls and remapping](controls-and-remapping.md)).
...
**Devices and players.** One to four players take turns in slot order: player 1, 2, 3, 4, then back to player 1. A player on their own throws all four bags in a row. Two players may not share a keyboard layout or a controller. Any number can use touch or AI. See [the input model](../foundations/input-model.md).

**Sound.** Off by default in every match. With **Sound on**, three short synthesized tones play: one when the bag leaves the hand, a bright one on a hole, and a low one on a board or floor landing. The button always matches what is heard, even when pressed while the match is still loading. Watch's sound switch has no effect here ([sound](../cross-cutting/sound.md)).

**Reduced motion and graphics quality.** Reduced motion removes the camera punch on a hole, the impact effects, and squash and stretch; the throw's timing and scoring are unchanged. Toggling it mid-match applies at once. Lower graphics quality is not applied to Play ([the stage](../foundations/stage.md)).

**Accessibility.** The caption is an `aria-live` region. It announces the turn prompt with the shot, the charging prompt with the shot, the release grade and each landing. A shot change is announced as a new prompt. The drawn **RELEASE TIMING** meter and the reticle are visual only. The page's own screen-reader copy of the meter has only its labels, "Release timing" and "Release in green", with no position, so nothing tells a screen-reader user where the marker is relative to the window. The on-screen buttons are real buttons, and the pads take arrow keys when focused ([accessibility](../cross-cutting/accessibility.md)).

**Installed characters.** An installed card can be chosen in setup. If its pack has no connected rig, the match fails to open with "Error: {full card name} needs a connected character rig for direct play. Its existing poses remain available in Watch." An installed card that does open starts each turn on its favourite of the four shots, or on **Hole runner** ([Install character](../collection/install-character.md)).
...
- **Starting spots.** Players 1 to 4 start at x = 215, 345, 280 and 190, all inside the 165–350 line, so nobody moves when their first turn begins.
- **Where the thrower stands** is kept between their own turns. **The reticle** is not; it re-centres every turn. **The shot** is not either; it goes back to the character's favourite.
...
- **A tie** ends with **Draw**, and every tied player celebrates. This includes 0–0.
- **A solo match** has one player throwing four bags in a row. They always win, so the result reads "{name} wins" whatever they score.
- **Precision mode hides the shot.** Turning it on replaces the caption with **Precision mode** until the caption next changes, for example when the player charges or picks a shot.
...
- **The on-screen Release button** returns to **Hold / release** by itself when the throw is released automatically, and when the turn passes. Outside the player's own aiming or charging turn, and while the match loads, the button is disabled.
...
- **Fixed: the shot is shown, and the starting shot can be chosen again** (B-25). The caption names the shot, and each turn starts on one of the four selectable shots. Dan's old "blocker" starting style is gone.
- **Fixed: rule slips** (B-26). A Roll curls around nearby bags once, not once per bag. Players 3 and 4 start inside the line and no longer jump on their first turn. The AI's release nudge follows each thrower's own bags, so one AI thrower is no longer always on target while another is always 0.03 off.
- **Fixed: the caption after a pause** (B-27). At `3b4ec62` the scripted pass saw the caption stay on **Release in the green window** after a pause cancelled the charge. It now returns to the aiming prompt with the shot.
- **Fixed: Reduced motion and sound** (B-07, B-08). Toggling Reduced motion no longer restarts the match, and the sound button always matches the sound.
- **Fixed: graphics loss** (B-14). A lost WebGL context now pauses the match, as described above. This is read from the code; it has not been tried on the page.
