...
**Sound.** Off by default in every match. With **Sound on**, three short synthesized tones play: one when the bag leaves the hand, a bright one on a hole, and a low one on a board or floor landing. Watch's sound switch has no effect here ([sound](../cross-cutting/sound.md)).

**Reduced motion and graphics quality.** Reduced motion removes the camera punch on a hole, the impact effects, and squash and stretch; the throw's timing and scoring are unchanged. Lower graphics quality is not applied to Play ([the stage](../foundations/stage.md)).

**Accessibility.** The caption is an `aria-live` region. It announces the turn prompt, **Release in the green window**, the release grade and each landing. The drawn **RELEASE TIMING** meter and the reticle are visual only. The page's own screen-reader copy of the meter has only its labels, "Release timing" and "Release in green", with no position, so nothing tells a screen-reader user where the marker is relative to the window. The on-screen buttons are real buttons, and the pads take arrow keys when focused ([accessibility](../cross-cutting/accessibility.md)).
...
- **The caption after a pause.** When a pause cancels a charge, the caption still reads **Release in the green window** after resuming. This was seen in the scripted pass, and looks like a bug (bug-triage B-27).
