...
export function pathAt(a:Attempt,seconds:number):V3 {const path=a.trajectory;const t=Math.max(0,seconds);let i=path.findIndex(p=>p.t>=t);if(i<0)return path[path.length-1];if(i===0)return path[0];const p=path[i-1],q=path[i],f=(t-p.t)/(q.t-p.t);return{x:p.x+(q.x-p.x)*f,y:p.y+(q.y-p.y)*f,z:p.z+(q.z-p.z)*f};}
