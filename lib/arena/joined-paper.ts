...
export class JoinedPaper {
 readonly mesh:MeshPlane;private texture:Texture;private binds:Bind[]=[];private curves:import('./puppet-geometry').SkinPiece[]=[];
 constructor(readonly skin:JoinedSkin,atlas:Texture){
  this.texture=new Texture({source:atlas.source,frame:new Rectangle(...skin.rect)});
  this.mesh=new MeshPlane({texture:this.texture,verticesX:NX,verticesY:NY});this.mesh.tint=0xfff2df;
  ...
  const indices=this.mesh.geometry.indices,triangles=[] as {indices:number[];depth:number}[];
  ...
  triangles.sort((a,b)=>a.depth-b.depth);this.mesh.geometry.indices=new Uint32Array(triangles.flatMap(t=>t.indices));
 }
 apply(p:PuppetPose,j:ReturnType<typeof puppetJoints>){
  const angle=p.body*Math.PI/180,ca=Math.cos(angle),sa=Math.sin(angle),positions=this.mesh.geometry.positions;
  ...
  for(let i=0;i<this.binds.length;i++){
   ...
   if(b.weight>0){
    const limb=b.arm?j.rightArm:j.leftArm,curved=skinVertex(-b.cross,b.arc,this.curves[b.arm],limb.root,limb.joint,limb.end);
    bx+=(curved.x-bx)*b.weight;by+=(curved.y-by)*b.weight;
   }
   positions[i*2]=bx;positions[i*2+1]=by;
  }
  this.mesh.geometry.getAttribute('aPosition').buffer.update();
 }
 destroy(){this.mesh.destroy();this.texture.destroy();}
}
